...
public:
	...
	virtual uint16_t readDeviceIdWord()=0;
};
...
public:
	...
	uint16_t readDeviceIdWord()
	{
		...
		programmer.pic_send_cmd(pic->load_configuration_cmd);
		programmer.pic_load_data(0x0000);
		for (i = 0; i < 6; i++)
			programmer.pic_send_cmd(pic->increment_address_cmd);
		programmer.pic_send_cmd(pic->read_data_from_program_memory_cmd);
...
const struct picmicro pic16f886 = {PIC16F886, "pic16f886", 0x2000, 256, 4000, 4000, 8000, 8000, 0x00, 0x02, 0x03, 0x04, 0x05, 0x06, 0x08, 0x18, 0x01, 0x07};

const struct picmicro *piclist[] = {&pic16f84a, &pic16f627a, &pic16f628a, &pic16f648a, &pic16f870, &pic16f871, &pic16f872, &pic16f873, &pic16f874, &pic16f876, &pic16f877, &pic16f886, NULL};
...
struct picmemory *read_inhx16(char *infile, int debug)
{
	...
	pm = (picmemory *)calloc(1, sizeof(*pm));
	if (pm)
	{
...
void write_inhx16(struct picmemory *pm, const char *outfile)
{
	FILE *fp;
...
void pic_read(const struct picmicro *pic, const char *outfile, int skipones, int debug)
{
	struct picmemory *pm;
	uint16_t addr, data;
